...
public class AIControls : MonoBehaviour
{
    ...
    void Awake()
    {
        // Initialize waypoints list by finding all Transform components that are children of waypointsHolder
        ...
        if (waypointsHolder != null)
        {
            ...
            {
                // Make sure not to add the transform of the holder itself
                if (waypoint != waypointsHolder)
                {
        ...
        {
            Debug.LogError("waypointsHolder not set on " + gameObject.name);
        }
    }
